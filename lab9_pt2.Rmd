---
title: "Part 2- Chi-squared lab table lizards talkes"
author: "Kiera Matiska"
date: "11/23/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

# Attach Packages
library(tidyverse)
library(here)
library(janitor)
library(broom)
```

## Read in data

```{r}
lizards <- read_csv(here("data", "lizards.csv"))
```

## Get counts of lizards by species

```{r}
lizards %>% 
  count(spp) %>% 
  arrange(-n)
```

## Make a subset

- UTST lizards
- Sites: IBPE (enclousure grassland), RABB (rabbit mesquite site), "EAST" (east tarbush site)
- Tail condition is record (W = whole, B = Broken)

```{r}
utst <- lizards %>% 
  filter(spp == "UTST") %>% 
  filter(site %in% c("IBPE", "RABB", "EAST")) %>% 
  filter(tail %in% c("B", "W"))
```

## Find counts and proportions of lizards with broken or whole tails at each site

```{r}
utst %>% 
  count(site, tail)
```

```{r}
tail_counts <- utst %>% 
  janitor::tabyl(site, tail)

tail_proportions <- tail_counts %>% 
  janitor::adorn_percentages() %>% 
  janitor::adorn_pct_formatting(digits = 2) %>% 
  janitor::adorn_ns()
```

## Convert to a contigency table

Use `column_to_rowname()`

```{r}
lizard_ct <- tail_counts %>% 
  column_to_rownames(var = "site")
```

## Chi-square

```{r}
lizard_x2 <- chisq.test(lizard_ct)

lizard_x2

lizard_tidy <- broom::tidy(lizard_x2)
```

There is a significant associations (i.e. non-indenpendence) between site and lizard tail breakage ($\chi$^2^(`r lizard_tidy$parameter`) = `r lizard_tidy$statistic`, "p" = `r lizard_tidy$p.value`)
